...
Class CustomerRewards extends Connection  {

    public function create($customer_id,$rewards_id,$points,$code){
        $db = $this->openConnection();
        $sql = "insert into `customer_rewards`(`customer_id`,`discount_id`,`points`,`code`) values ('$customer_id','$rewards_id','$points','$code')";
        $db->exec($sql);
        $this->closeConnection();
    }
    // public function update($customer_id,$rewards_id,$id){
    //     $db = $this->openConnection();
    //     $sql = "update `customer_rewards` set `title`  = '$title' , `code`  = '$code',`type`  = '$type',`value`  = '$value' , `points`  = '$points' where `id` = $id ";
    //     $db->exec($sql);
    //     $this->closeConnection();
    // }
    ...
    //     $this->closeConnection();
    // }
    public function getCustomerRewardsByCustomerId($id){
        $db = $this->openConnection();
        $stmt = $db->prepare('SELECT * FROM customer_rewards where customer_id = :id order by id desc  ');
        $stmt->execute(['id' => $id]);
        $data = $stmt->fetchAll();
        $this->closeConnection();
        return $data;
    ...
    public function getCustomerRewardsSumByCustomerId($id){
        $db = $this->openConnection();
        $stmt = $db->prepare('SELECT sum(points) as actual_point FROM customer_rewards where customer_id = :id ');
        $stmt->execute(['id' => $id]);
        $data = $stmt->fetch();
        $this->closeConnection();
    ...
    public function updateUsedByCode($code,$customer_id){
        $db = $this->openConnection();
        $sql = "update `customer_rewards` set `used`  = '1'  where `code` = '$code' and `customer_id` = $customer_id ";
        $db->exec($sql);
        $this->closeConnection();
    }
    public function addRewardsDataByCustomer($data_shop,$layoltyPoints){
        $shopify = 
        ...
        $actual_point = $this->getCustomerRewardsSumByCustomerId($data_shop['customer_id']);
        ...
        if($bonus_points >= $actual_point){
            ...
            $this->create($data_shop['customer_id'],0,$get_points,$code);
