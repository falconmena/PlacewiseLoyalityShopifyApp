...
var root_path = "https://ownframework.placewise-services.com"
	...
	if ((xmlhttp.status == 200) && (xmlhttp.readyState == 4)){
        ...
        function checkCustomerLoyaltyFound(){
            ...
            let data = {
                ...
                // id: customer_id,
                // shop: shop_url,
                r:`ShopifyApi/checkCustomerLoyaltyFound/${shop_url}/${customer_id}`
            };
            $.ajax({
                type: "GET",
                url: root_path,
                ...
                success: function (data) {
                    ...
                    if(data['properties'] != undefined){
                        ...
                        $('#cart_actual_points').text(data['properties']['actual_point']);
        ...
        function can_submit(){
            ...
            if(password_check != re_password_check){
              ...
              $('#error_msg').text("Ugyldig telefonnummer, eksempel gyldig nummer:+16135551212");
                ...
                let data = {
                    r: "shopifyApi/checkCanSubmitCreateAccount",
                    id: customer_id,
                    shop: shop_url,
        ...
        function addCustomerRewards(customer_id,shop_url,points){
            ...
            $.ajax({
                type: "POST",
                url: root_path + "/?r=shopifyApi/addRewardsDataByCustomer",
                data: data,
                dataType: "json",
                success: function (data) {
                    console.log(data)
        ...
        function getCustomerRewards(customer_id,shop_url){
            let data = {
                r: "shopifyApi/getCustomerRewards/" + shop_url + "/" + customer_id,
            };
        ...
        $('#techs_phone_number').on('keyup',function(){
            ...
            if(!validate_phone){
                ...
                let data = {
                  r:`ShopifyApi/checkCustomerPhoneLoyaltyFound/${shop_url}/${phone}`
                };
                $.ajax({
                  type: "GET",
                  url: root_path,
        ...
        function check_add_phone(){
            let customer_id = getCustomerId();  
            let shop_url = $('#data-shop-name').attr('data-shop');
            $('#error_phone_loyalty').text("");
            let validate_phone = validate($('#input-phone').val())
            if(!validate_phone){
                $('#error_phone_loyalty').text("Vennligst skriv inn gyldig telefonnummer");
            }else{
        ...
        $('#submit_loyalty_system_account').on('click',function(e){
            ...
            $.ajax({
                type: "POST",
                url: root_path + "/?r=ShopifyApi/editMember",
                data: data,
                dataType: "json",
                success: function (data) {
        ...
        $('#submit_loyalty_system_add_account').on('click',function(e){
            ...
            let gender = $('input[name="gender"]:checked').val();
            let phone = $('#input-phone').val();
            let validate_phone = validate(phone)
            let birthday = $('#input-birthday').val();
            let sms = $('input[name="input-sms"]:checked').val();
            let email = $('input[name="input-email"]:checked').val();
            let offers = $('input[name="input-offers"]:checked').val();
            ...
            if(!validate_phone){
              $('#error_msg_add_member').text("Ugyldig telefonnummer, eksempel gyldig nummer:+16135551212");
            }else{
                $.ajax({
                type: "POST",
                url: root_path + "/?r=ShopifyApi/addMember",
                data: data,
                dataType: "json",
                success: function (data) {
                    console.log(data)
